...
def parse_ibk_excel(xls_path: Path):
    """campus.ibk.co.kr '주차별 출석부 상세' 화면의 '엑셀다운' 결과(xls)를 파싱한다.

    이 xls는 병합 셀 없이 완전한 사각형 그리드로 내려오지만, colspan을 흉내
    내듯 그룹의 첫 칸에만 라벨("1주" 등)이 있고 나머지는 빈 문자열이다.
    렌더링된 HTML 표를 colspan/rowspan까지 감안해 긁어야 하는 것과 달리,
    브라우저에서 실제로 보이는 테이블을 잘못 고르는 문제(SPA가 이전 화면의
    테이블을 DOM에 숨겨서 남겨두는 경우 등)에서 완전히 자유롭다.
    """
    import xlrd
    ...
    for r in range(sheet.nrows):
        if _cell_str(sheet.cell_value(r, 0)) == "NO":
            header_row = r
            break
    ...
    # "1주"/"2주" 같은 라벨이 그룹의 첫 칸에만 있고 나머지는 빈 문자열이므로,
    # 빈 칸을 직전 라벨로 채워야 각 열이 몇 주차인지 알 수 있다.
    filled_header = []
    last = ""
    for c in range(ncols):
        text = _cell_str(sheet.cell_value(header_row, c))
        if text:
            last = text
        filled_header.append(last)

    columns = []
    week_item_counters = {}
    for c in range(ncols):
        raw = _cell_str(sheet.cell_value(header_row, c))
        if raw == "주":
            continue  # "월/일/요일/시간" 하위 표를 가리키는 라벨 칸일 뿐, 데이터가 없음
        m = WEEK_RE.search(filled_header[c])
        if m:
            week_num = int(m.group(1))
            week_item_counters[week_num] = week_item_counters.get(week_num, 0) + 1
            columns.append({"col": c, "kind": "week", "week": week_num, "item_index": week_item_counters[week_num]})
        else:
            columns.append({"col": c, "kind": "meta", "label": raw or f"col_{c}"})

    meta_labels = [col["label"] for col in columns if col["kind"] == "meta"]
    long_rows = []
    for r in range(data_start, sheet.nrows):
        if not _cell_str(sheet.cell_value(r, 0)):
            continue

        meta = {
            col["label"]: _cell_str(sheet.cell_value(r, col["col"]))
            for col in columns
            if col["kind"] == "meta"
        }
        ...
        for col in columns:
            ...
            entry["주차"] = col["week"]
            entry["항목순번"] = col["item_index"]
            entry["출결상태"] = _cell_str(sheet.cell_value(r, col["col"]))
            long_rows.append(entry)

    week_cols = [col for col in columns if col["kind"] == "week"]
    weeks = sorted(set(col["week"] for col in week_cols))
    items_per_week = {w: sum(1 for col in week_cols if col["week"] == w) for w in weeks}
    student_count = sum(1 for r in range(data_start, sheet.nrows) if _cell_str(sheet.cell_value(r, 0)))
