...
- **Decision**: 단일 "다음 이벤트" 시간 기반 트리거를 매 실행 종료 시 삭제 후 다음 가장 가까운
  시작/종료 시각으로 재생성한다(FR-017). 이와 별도로 (a) 채점중/평가완료 행을 점검하는 30분
  주기 트리거 1개(교사가 평가를 언제 입력할지 예측할 수 없어 "다음 이벤트" 계산 대상이 아니므로
  구현 단계에서 필요성이 드러나 추가함), (b) 일일 요약 메일 전용 고정 시각 트리거 1개, (c) 교사가
  일정/진행상태를 수정했을 때 재계산을 유발하는 `onEdit` 설치형 트리거 1개를 둔다. 총 시간 기반
  트리거는 3개(동적 이벤트 + 채점 파이프라인 주기 점검 + 일일 요약)이며 `onEdit`은 시간 기반이
  아니므로 헌법의 "8개 이하" 한도에 여유가 크다.
